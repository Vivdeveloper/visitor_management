...
function add_gate_actions(frm) {
	...
	// View auto-generated gate pass (created on Visitor Entry insert)
	if (frm.doc.status !== "Rejected" && frm.doc.status !== "Cancelled") {
		frm.add_custom_button(__("View Gate Pass"), () => view_gate_pass(frm), __("Gate"));
	}

	// 2) Approved → Check In (pass already exists; refreshed on check-in)
...
function view_gate_pass(frm) {
	frappe.call({
		method: `${VE}.generate_pass`,
		args: { visitor_entry: frm.doc.name },
		freeze: true,
		callback(r) {
			if (r.exc) return;
			const pass_url = r.message && r.message.pass_url;
			if (!pass_url) {
				frappe.msgprint({
					title: __("Gate Pass"),
					message: __("Gate pass was not generated by the server."),
					indicator: "red",
				});
				return;
			}
			...
			frappe.msgprint({
				title: __("Gate Pass"),
				message: __(
					"Gate pass for <b>{0}</b>:<br><br><a href='{1}' target='_blank' rel='noopener'>{1}</a>",
					[frm.doc.full_name || frm.doc.name, pass_url]
				),
				indicator: "blue",
			});
			window.open(pass_url, "_blank");
		},
	});
}
